...
GameObject::GameObject(GameObjectType t) 
    : type(t), color(1.0f), transparency(1.0f), materialType(0), isActive(true), isTrigger(false), useSphereCollision(false) {
}
...
void GameObject::updateBoundingSphere(float radius) {
    boundingSphere = Physics::createSphereFromTransform(transform.position, radius);
}
...
void CrumblingTile::update(float deltaTime) {
    if (!isTriggered || hasFallen) return;
    ...
    if (shakeTimer < 1.0f) {
        ...
        transform.position.x = originalPosition.x + (sin(shakeTimer * 20.0f) * shakeAmount);
        transform.position.z = originalPosition.z + (cos(shakeTimer * 15.0f) * shakeAmount);
    } else {
        // Fall
...
// Stalactite Implementation
Stalactite::Stalactite(const glm::vec3& position)
    : GameObject(GameObjectType::STALACTITE), isFalling(false), fallSpeed(0.0f) {
...
Geyser::Geyser(const glm::vec3& position)
    : GameObject(GameObjectType::GEYSER), eruptTimer(0.0f),
...
void Geyser::draw(Shader* shader) {
    ...
    if (isErupting) {
        shader->setVec3("objectColor", glm::vec3(0.5f, 0.4f, 0.3f)); // Lighter when erupting
...
Collectible::Collectible(const glm::vec3& position, const glm::vec3& color) 
    ...
    mesh.reset(Mesh::createSphere(0.5f, 8, 8)); // Low poly sphere looks like gem
    updateBoundingSphere(0.5f);
...
void Collectible::update(float deltaTime) {
    if (isCollected) {
        ...
        // Spin and shrink
        transform.rotate(deltaTime * rotationSpeed * 5.0f, glm::vec3(0.0f, 1.0f, 0.0f));
        ...
        // Update initial scale if not collected (to capture any external scale changes)
        if (collectAnimation == 0.0f) {
             // This is tricky. If we update initialScale here, we might overwrite external sets.
             // Better to assume external code sets transform.scale and we capture it once or respect it.
             // Actually, let's just use transform.scale as initialScale when collect() is called.
        }
    }
    
    updateBoundingSphere(0.3f * transform.scale.x);
}
