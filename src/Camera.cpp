...
void Camera::processMouseMovement(float xoffset, float yoffset,
  ...
  if (constrainPitch) {
    if (mode == THIRD_PERSON) {
      ...
      if (pitch > 89.0f)
        pitch = 89.0f;
      if (pitch < -10.0f)
        pitch = -10.0f; // Don't look up from below ground
    } else {
      if (pitch > 89.0f)
        pitch = 89.0f;
      if (pitch < -89.0f)
  ...
  // In Third Person, vectors are updated in setThirdPersonMode based on player
  // pos
}
...
void Camera::setThirdPersonMode(const glm::vec3 &playerPos) {
  ...
  // If pitch is negative (looking down), camera should be ABOVE (positive Y
  // offset) So we negate the Y component of the offset calculation effectively
  // Let's simplify:
