...
void Player::update(float deltaTime, const glm::vec3& moveInput) {
    ...
    glm::vec3 targetVelocity = glm::vec3(moveInput.x, 0.0f, moveInput.z) * moveSpeed;
    ...
    if (glm::length(moveInput) > 0.0f) {
        // Accelerate
        velocity.x = glm::mix(velocity.x, targetVelocity.x, acceleration * deltaTime);
        velocity.z = glm::mix(velocity.z, targetVelocity.z, acceleration * deltaTime);
    } else {
        // Decelerate (Friction)
...
void Player::draw(Shader* shader) {
    ...
    glm::vec3 coreColor = isFlashing ? glm::vec3(1.0f, 0.0f, 0.0f) : glm::vec3(0.8f, 0.7f, 0.2f);
    ...
    headModel = glm::translate(headModel, glm::vec3(0.0f, 0.6f, 0.0f)); // Float above
    ...
    glm::vec3 fragmentColor = glm::vec3(0.4f, 0.8f, 1.0f); // Glowing blue energy stones
...
void Player::onWallCollision(const glm::vec3& normal, ParticleSystem* particles) {
    ...
    if (particles) {
        particles->emit(
            transform.position + normal * 0.6f,
            normal * 3.0f,
...
void Player::onObstacleHit(const glm::vec3& knockbackDir, ParticleSystem* particles) {
    // Strong knockback
